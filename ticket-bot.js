/* ITExpresSolutions - Bot de tickets sin IA
   Flujo: abrir ticket + consultar ticket.
...
(function(){
  function boot(){
    ...
    function start(){
      ...
      setQuick([
        {label:'🎫 Abrir un ticket',value:'open'},
        {label:'🔎 Consultar mi ticket',value:'lookup'}
      ]);
    }
    ...
    function startOpenTicket(){
      ...
      setQuick([
        {label:'México',value:'México',country:'México'},
        {label:'Costa Rica',value:'Costa Rica',country:'Costa Rica'}
      ]);
    }
    ...
    async function createTicket(){
      ...
      state.data.ticketId=ticketId;
      const ref=ticketId.slice(0,8).toUpperCase()||'REGISTRADO';
      state.step='done';
      addMessage('✅ Ticket creado correctamente.');
      addMessage('Número de referencia: #'+ref);
      addMessage('Guarda este número. Lo necesitarás para consultar el estado de tu ticket.');
      setQuick([{label:'🔎 Consultar este ticket',value:'lookup_now'},{label:'↻ Abrir otro ticket',value:'restart'}]);
    }
    ...
    async function lookupTicket(){
      state.step='lookup_sending';setQuick([]);addMessage('Consultando tu ticket…');
      const sb=window.ITExpresSupabase;
      if(!sb){addMessage('No se pudo conectar con el sistema de tickets. Intenta nuevamente más tarde.');return;}
      ...
      if(error){console.error('consultar_ticket_web:',error);state.step='lookup_error';addMessage('No encontré un ticket con esos datos. Verifica la referencia y el medio de contacto e inténtalo nuevamente.');setQuick([{label:'🔎 Intentar otra vez',value:'lookup'},{label:'🏠 Inicio',value:'home'}]);return;}
      const t=data||{};
      state.step='lookup_done';
      ...
      setQuick([{label:'🔎 Consultar otro ticket',value:'lookup'},{label:'🏠 Inicio',value:'home'}]);
    }
    ...
    function handleText(text){
      ...
      if(state.step==='menu'){addMessage('Selecciona si quieres abrir un ticket o consultar uno existente.');setQuick([{label:'🎫 Abrir un ticket',value:'open'},{label:'🔎 Consultar mi ticket',value:'lookup'}]);return;}
